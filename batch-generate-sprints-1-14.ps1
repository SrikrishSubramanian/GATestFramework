...
<#
.SYNOPSIS
FAST BATCH GENERATION - Sprints 1-14 (400+ tickets)
Runs 5 parallel workers for maximum speed
#>
...
Write-Host ""
Write-Host "╔════════════════════════════════════════════════════════╗"
Write-Host "║   BATCH JIRA TEST GENERATION - SPRINTS 1-14            ║"
Write-Host "║   $totalTickets Tickets | 5 Parallel Workers          ║"
Write-Host "╚════════════════════════════════════════════════════════╝"
Write-Host ""
Write-Host "STRATEGY: Parallel processing with 5 workers" -ForegroundColor Green
Write-Host "Expected time: 6-8 hours for all $totalTickets tickets" -ForegroundColor Cyan
Write-Host "Environment: DEV" -ForegroundColor Cyan
Write-Host ""

# Process in parallel with throttle limit of 5
$results = $tickets | ForEach-Object -Parallel {
    $ticket = $_
    $index = $using:tickets.IndexOf($ticket) + 1
    $total = $using:totalTickets

    Write-Host "[$index/$total] Generating $ticket..." -ForegroundColor Cyan
    ...
    try {
        ...
        # Execute the Playwright generator
        & npx playwright test generate-from-jira --config playwright.generators.config.ts --project chromium 2>&1

        Write-Host "[$index/$total] ✓ $ticket - Complete" -ForegroundColor Green
        return @{ ticket = $ticket; status = "success" }
    }
    catch {
        Write-Host "[$index/$total] ✗ $ticket - Failed: $_" -ForegroundColor Red
        return @{ ticket = $ticket; status = "failed"; error = $_ }
    }
} -ThrottleLimit 5

# Count results
$results | ForEach-Object {
    if ($_.status -eq "success") { $successCount++ } else { $failureCount++ }
}
...
Write-Host "  ✓ Successful: $successCount/$totalTickets" -ForegroundColor Green
Write-Host "  ✗ Failed: $failureCount/$totalTickets" -ForegroundColor Red
...
Write-Host "  1. Run smoke tests: env=dev npx playwright test tests/specFiles/ga/ --grep '@smoke' --project chromium" -ForegroundColor White
Write-Host "  2. Check coverage: ls tests/specFiles/ga/*/**.spec.ts | wc -l" -ForegroundColor White
Write-Host "  3. View reports: start playwright-report/index.html" -ForegroundColor White
Write-Host ""
