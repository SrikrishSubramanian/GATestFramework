...
# Batch Jira Generation - Sprints 1-14

$allTickets = @(
    'GAAM-961','GAAM-960','GAAM-949','GAAM-948','GAAM-947','GAAM-945','GAAM-944','GAAM-943','GAAM-942','GAAM-941',
    'GAAM-939','GAAM-937','GAAM-926','GAAM-923','GAAM-914','GAAM-912','GAAM-911','GAAM-910','GAAM-909','GAAM-902',
    ...
    'GAAM-54','GAAM-52','GAAM-43','GAAM-26','GAAM-25','GAAM-24','GAAM-23'
)
...
Write-Host ""
Write-Host "BATCH JIRA GENERATION - SPRINTS 1-14"
Write-Host "Total: $totalTickets tickets | 5 parallel workers"
...
# Sequential processing (more stable than parallel for large batches)
foreach ($ticket in $allTickets) {
    $index = $allTickets.IndexOf($ticket) + 1
    ...
    try {
        ...
        $failureCount++
        Write-Host "[$index/$totalTickets] FAIL - $ticket" -ForegroundColor Red
    }
