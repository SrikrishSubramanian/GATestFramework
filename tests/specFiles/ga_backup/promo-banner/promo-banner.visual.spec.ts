...
test.describe('PromoBanner — Visual Regression', () => {
  test('@visual @regression Desktop screenshot matches baseline', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    const pom = new PromoBannerPage(page);
    await pom.navigate(BASE());
    const el = page.locator('.cmp-promo-banner').first();
    await expect(el).toHaveScreenshot('promo-banner-desktop.png', {
  ...
  test('@visual @regression @mobile Mobile screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-promo-banner').first();
    await expect(el).toHaveScreenshot('promo-banner-mobile.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
  ...
  test('@visual @regression Tablet screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-promo-banner').first();
    await expect(el).toHaveScreenshot('promo-banner-tablet.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
