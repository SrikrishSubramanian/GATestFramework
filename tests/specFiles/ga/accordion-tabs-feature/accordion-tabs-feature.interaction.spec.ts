    ...
    test('[ATF-058] @interaction @regression CSS transition/animation exists on tab panels', async ({ page }) => {
        ...
        const hasTransition = // 📏 TODO: Replace with measurement-utils
         await instance.evaluate(root => {
            const elements = root.querySelectorAll('.cmp-accordion-tabs-feature__accordion-body, .cmp-accordion-tabs-feature__accordion-header, .cmp-accordion-tabs-feature__accordion-item, .cmp-accordion-tabs-feature__image-panel');
...
test.describe('AccordionTabsFeature — Dark Background Interaction @interaction @regression', () => {
    test('[ATF-078] @interaction @regression Dark bg: accordion tabs are interactive on granite background', async ({ page }) => {
        ...
        if (graniteCount === 0) {
            test.skip();
            return;
        } // Granite fixture not deployed
        const darkInstance = graniteSections.first().locator(ROOT);
        if (await darkInstance.count() === 0) {
            test.skip();
            return;
        }
        ...
        for (let i = 0; i < tabCount; i++) {
            ...
            await tabs.nth(i).click();
            if (wasSelected) {
                const states = await Promise.all(Array.from({ length: tabCount }, (_, j) => tabs.nth(j).getAttribute('aria-selected')));
    ...
    test('[ATF-079] @interaction @regression Dark bg: text is readable (light color on dark background)', async ({ page }) => {
        ...
        await pom.navigate(BASE());
        const graniteSections = page.locator('.cmp-section--background-color-granite');
        if (await graniteSections.count() === 0) {
            test.skip();
            return;
        }
        const darkInstance = graniteSections.first().locator(ROOT);
