...
test.describe('BioCard — Accessibility', () => {
  test('[BC-010] @a11y @wcag22 @regression @smoke BioCard passes axe-core scan', async ({ page }) => {
    const pom = new BioCardPage(page);
    await pom.navigate(BASE());
    const results = await new AxeBuilder({ page })
      .include('.cmp-bio-card')
      .withTags(["wcag2a","wcag2aa","wcag22aa"])
      .analyze();
    expect(results.violations).toEqual([]);
  });

  test('[BC-011] @a11y @wcag22 @regression @smoke BioCard interactive elements meet 24px target size', async ({ page }) => {
    const pom = new BioCardPage(page);
    await pom.navigate(BASE());
    const interactive = page.locator('.cmp-bio-card a, .cmp-bio-card button, .cmp-bio-card input');
    const count = await interactive.count();
    for (let i = 0; i < count; i++) {
  ...
  test('[BC-012] @a11y @wcag22 @regression @smoke BioCard focus is not obscured by sticky elements', async ({ page }) => {
    const pom = new BioCardPage(page);
    await pom.navigate(BASE());
    const focusable = page.locator('.cmp-bio-card a, .cmp-bio-card button, .cmp-bio-card input');
...
test.describe('BioCard — AEM Dialog Configuration', () => {
  ...
  test('[BC-013] @author @regression @smoke @smoke BioCard dialog has helpPath configured', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/bio-card/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
    expect(response.ok(), 'BioCard GA dialog overlay not found — component may be missing _cq_dialog').toBe(true);
    const dialog = await response.json();
    expect(dialog.helpPath, 'BioCard dialog missing helpPath property').toBeTruthy();
  });

  test('[BC-014] @author @regression @smoke BioCard helpPath points to correct component details page', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/bio-card/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
