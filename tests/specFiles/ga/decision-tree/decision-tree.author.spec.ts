...
test.describe('DecisionTree — Broken Images', () => {
  test('[DT-008] @regression DecisionTree all images load successfully', async ({ page }) => {
    const pom = new DecisionTreePage(page);
    await pom.navigate(BASE());
    const images = page.locator('.cmp-decision-tree img');
    const count = await images.count();
...
test.describe('DecisionTree — Accessibility', () => {
  test('[DT-010] @a11y @wcag22 @regression @smoke DecisionTree passes axe-core scan', async ({ page }) => {
    const pom = new DecisionTreePage(page);
    await pom.navigate(BASE());
    const results = await new AxeBuilder({ page })
      .include('.cmp-decision-tree')
      .withTags(["wcag2a","wcag2aa","wcag22aa"])
      .analyze();
    expect(results.violations).toEqual([]);
  });

  test('[DT-011] @a11y @wcag22 @regression @smoke DecisionTree interactive elements meet 24px target size', async ({ page }) => {
    const pom = new DecisionTreePage(page);
    await pom.navigate(BASE());
    const interactive = page.locator('.cmp-decision-tree a, .cmp-decision-tree button, .cmp-decision-tree input');
    const count = await interactive.count();
    for (let i = 0; i < count; i++) {
  ...
  test('[DT-012] @a11y @wcag22 @regression @smoke DecisionTree focus is not obscured by sticky elements', async ({ page }) => {
    const pom = new DecisionTreePage(page);
    await pom.navigate(BASE());
    const focusable = page.locator('.cmp-decision-tree a, .cmp-decision-tree button, .cmp-decision-tree input');
...
test.describe('DecisionTree — AEM Dialog Configuration', () => {
  ...
  test('[DT-013] @author @regression @smoke @smoke DecisionTree dialog has helpPath configured', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/decision-tree/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
    expect(response.ok(), 'DecisionTree GA dialog overlay not found — component may be missing _cq_dialog').toBe(true);
    const dialog = await response.json();
    expect(dialog.helpPath, 'DecisionTree dialog missing helpPath property').toBeTruthy();
  });

  test('[DT-014] @author @regression @smoke DecisionTree helpPath points to correct component details page', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/decision-tree/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
