...
test.describe('ImageWithNestedContent — Section Background Colors', () => {
    test('[IWNC-029] @regression White section: content-trail bg inherits white', async ({ page }) => {
        ...
        await ctInstance.evaluate(el => {
            ...
            if (wrapper)
                wrapper.classList.add('cmp-section--background-color-white');
        });
        ...
        await ctInstance.evaluate(el => {
            const wrapper = el.closest('.aem-GridColumn') || el.parentElement;
            if (wrapper)
                wrapper.classList.remove('cmp-section--background-color-white');
        });
    });
    ...
    test('[IWNC-031] @regression Granite section: statistic bg inherits granite', async ({ page }) => {
        ...
        await statInstance.evaluate(el => {
            const wrapper = el.closest('.aem-GridColumn') || el.parentElement;
            if (wrapper)
                wrapper.classList.add('cmp-section--background-color-granite');
        });
        const bg = await statInstance.locator(STAT_ITEM).first().evaluate(el => getComputedStyle(el).backgroundColor);
        // measurement: use measurement-utils for cleaner code
        ...
        await statInstance.evaluate(el => {
            const wrapper = el.closest('.aem-GridColumn') || el.parentElement;
            if (wrapper)
                wrapper.classList.remove('cmp-section--background-color-granite');
        });
    });
    test('[IWNC-032] @regression Azul section: statistic text is white', async ({ page }) => {
        ...
        await statInstance.evaluate(el => {
            const wrapper = el.closest('.aem-GridColumn') || el.parentElement;
            if (wrapper)
                wrapper.classList.add('cmp-section--background-color-azul');
        });
        const descColor = await statInstance.locator('.cmp-statistic__description p').first().evaluate(el => getComputedStyle(el).color);
        expect(descColor).toMatch(/rgb\(255,\s*255,\s*255\)/);
        // 📏 TODO: Replace with measurement-utils
        await statInstance.evaluate(el => {
            const wrapper = el.closest('.aem-GridColumn') || el.parentElement;
            if (wrapper)
                wrapper.classList.remove('cmp-section--background-color-azul');
        });
    });
    test('[IWNC-033] @regression Granite section: statistic value uses themed color', async ({ page }) => {
        ...
        const color = await statInstance.locator('.cmp-statistic__value').first().evaluate(el => getComputedStyle(el).color);
