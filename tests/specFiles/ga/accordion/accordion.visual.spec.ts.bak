...
test.describe('Accordion — Visual Regression', () => {
  test('@visual @regression Desktop screenshot matches baseline', async ({ page }) => {
    ...
    const el = page.locator('.cmp-accordion').first();
    await expect(el).toHaveScreenshot('accordion-desktop.png', {
  ...
  test('@visual @regression @mobile Mobile screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-accordion').first();
    await expect(el).toHaveScreenshot('accordion-mobile.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
  ...
  test('@visual @regression Tablet screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-accordion').first();
    await expect(el).toHaveScreenshot('accordion-tablet.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
