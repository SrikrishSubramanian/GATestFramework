...
test.describe('Header — Console & Resources', () => {
  test('[HDR-019] @regression Header produces no JS errors', async ({ page }) => {
...
test.describe('Header — Broken Images', () => {
  ...
  test('[HDR-021] @regression Header all images have alt attributes', async ({ page }) => {
    const pom = new HeaderPage(page);
    await pom.navigate(BASE());
    const images = page.locator('.cmp-header img');
    const count = await images.count();
...
test.describe('Header — AEM Dialog Configuration', () => {
  ...
  test('[HDR-025] @author @regression @smoke @smoke Header dialog has helpPath configured', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/header/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
    expect(response.ok(), 'Header GA dialog overlay not found — component may be missing _cq_dialog').toBe(true);
    const dialog = await response.json();
    expect(dialog.helpPath, 'Header dialog missing helpPath property').toBeTruthy();
  });

  test('[HDR-026] @author @regression @smoke Header helpPath points to correct component details page', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/header/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
