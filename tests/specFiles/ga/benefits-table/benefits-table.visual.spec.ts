...
test.describe('BenefitsTable — Visual Regression', () => {
  test('@visual @regression Desktop screenshot matches baseline', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    const pom = new BenefitsTablePage(page);
    await pom.navigate(BASE());
    const el = page.locator('.cmp-benefits-table').first();
    await expect(el).toHaveScreenshot('benefits-table-desktop.png', {
  ...
  test('@visual @regression @mobile Mobile screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-benefits-table').first();
    await expect(el).toHaveScreenshot('benefits-table-mobile.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
  ...
  test('@visual @regression Tablet screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-benefits-table').first();
    await expect(el).toHaveScreenshot('benefits-table-tablet.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
