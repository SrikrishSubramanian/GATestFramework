...
test.describe('HomepageHero — Visual Regression', () => {
  test('@visual @regression Desktop screenshot matches baseline', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    const pom = new HomepageHeroPage(page);
    await pom.navigate(BASE());
    const el = page.locator('.cmp-homepage-hero').first();
    await expect(el).toHaveScreenshot('homepage-hero-desktop.png', {
  ...
  test('@visual @regression @mobile Mobile screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-homepage-hero').first();
    await expect(el).toHaveScreenshot('homepage-hero-mobile.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
  ...
  test('@visual @regression Tablet screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-homepage-hero').first();
    await expect(el).toHaveScreenshot('homepage-hero-tablet.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
