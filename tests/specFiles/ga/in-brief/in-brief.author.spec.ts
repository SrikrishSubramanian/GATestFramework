...
test.describe('InBrief — Accessibility', () => {
  test('[IB-010] @a11y @wcag22 @regression @smoke InBrief passes axe-core scan', async ({ page }) => {
    const pom = new InBriefPage(page);
    await pom.navigate(BASE());
    const results = await new AxeBuilder({ page })
      .include('.cmp-in-brief')
      .withTags(["wcag2a","wcag2aa","wcag22aa"])
      .analyze();
    expect(results.violations).toEqual([]);
  });

  test('[IB-011] @a11y @wcag22 @regression @smoke InBrief interactive elements meet 24px target size', async ({ page }) => {
    const pom = new InBriefPage(page);
    await pom.navigate(BASE());
    const interactive = page.locator('.cmp-in-brief a, .cmp-in-brief button, .cmp-in-brief input');
    const count = await interactive.count();
    for (let i = 0; i < count; i++) {
  ...
  test('[IB-012] @a11y @wcag22 @regression @smoke InBrief focus is not obscured by sticky elements', async ({ page }) => {
    const pom = new InBriefPage(page);
    await pom.navigate(BASE());
    const focusable = page.locator('.cmp-in-brief a, .cmp-in-brief button, .cmp-in-brief input');
...
test.describe('InBrief — AEM Dialog Configuration', () => {
  ...
  test('[IB-013] @author @regression @smoke @smoke InBrief dialog has helpPath configured', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/in-brief/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
    expect(response.ok(), 'InBrief GA dialog overlay not found — component may be missing _cq_dialog').toBe(true);
    const dialog = await response.json();
    expect(dialog.helpPath, 'InBrief dialog missing helpPath property').toBeTruthy();
  });

  test('[IB-014] @author @regression @smoke InBrief helpPath points to correct component details page', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/in-brief/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
