...
test.describe('GatedSection — Visual Regression', () => {
  test('@visual @regression Desktop screenshot matches baseline', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    const pom = new GatedSectionPage(page);
    await pom.navigate(BASE());
    const el = page.locator('.cmp-gated-section').first();
    await expect(el).toHaveScreenshot('gated-section-desktop.png', {
  ...
  test('@visual @regression @mobile Mobile screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-gated-section').first();
    await expect(el).toHaveScreenshot('gated-section-mobile.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
  ...
  test('@visual @regression Tablet screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-gated-section').first();
    await expect(el).toHaveScreenshot('gated-section-tablet.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
