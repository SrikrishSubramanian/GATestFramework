...
test.describe('GatedSection — Responsive', () => {
  ...
  test('[GS-006] @mobile @regression GatedSection adapts to tablet viewport', async ({ page }) => {
...
test.describe('GatedSection — Broken Images', () => {
  ...
  test('[GS-009] @regression GatedSection all images have alt attributes', async ({ page }) => {
    const pom = new GatedSectionPage(page);
    await pom.navigate(BASE());
    const images = page.locator('.cmp-gated-section img');
    const count = await images.count();
...
test.describe('GatedSection — Accessibility', () => {
  test('[GS-010] @a11y @wcag22 @regression @smoke GatedSection passes axe-core scan', async ({ page }) => {
    const pom = new GatedSectionPage(page);
    await pom.navigate(BASE());
    const results = await new AxeBuilder({ page })
      .include('.cmp-gated-section')
      .withTags(["wcag2a","wcag2aa","wcag22aa"])
      .analyze();
    expect(results.violations).toEqual([]);
  });

  test('[GS-011] @a11y @wcag22 @regression @smoke GatedSection interactive elements meet 24px target size', async ({ page }) => {
    const pom = new GatedSectionPage(page);
    await pom.navigate(BASE());
    const interactive = page.locator('.cmp-gated-section a, .cmp-gated-section button, .cmp-gated-section input');
    const count = await interactive.count();
    for (let i = 0; i < count; i++) {
  ...
  test('[GS-012] @a11y @wcag22 @regression @smoke GatedSection focus is not obscured by sticky elements', async ({ page }) => {
    const pom = new GatedSectionPage(page);
    await pom.navigate(BASE());
    const focusable = page.locator('.cmp-gated-section a, .cmp-gated-section button, .cmp-gated-section input');
...
test.describe('GatedSection — AEM Dialog Configuration', () => {
  ...
  test('[GS-013] @author @regression @smoke @smoke GatedSection dialog has helpPath configured', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/gated-section/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
    expect(response.ok(), 'GatedSection GA dialog overlay not found — component may be missing _cq_dialog').toBe(true);
    const dialog = await response.json();
    expect(dialog.helpPath, 'GatedSection dialog missing helpPath property').toBeTruthy();
  });

  test('[GS-014] @author @regression @smoke GatedSection helpPath points to correct component details page', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/gated-section/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
