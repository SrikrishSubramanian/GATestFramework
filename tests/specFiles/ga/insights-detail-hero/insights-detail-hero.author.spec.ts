...
test.describe('InsightsDetailHero — Negative & Boundary', () => {
  ...
  test('[IDH-004] @negative @regression InsightsDetailHero handles missing images', async ({ page }) => {
    const pom = new InsightsDetailHeroPage(page);
    await pom.navigate(BASE());
...
test.describe('InsightsDetailHero — Responsive', () => {
  ...
  test('[IDH-006] @mobile @regression InsightsDetailHero adapts to tablet viewport', async ({ page }) => {
...
test.describe('InsightsDetailHero — Broken Images', () => {
  test('[IDH-008] @regression InsightsDetailHero all images load successfully', async ({ page }) => {
    const pom = new InsightsDetailHeroPage(page);
    await pom.navigate(BASE());
    const images = page.locator('.cmp-insights-detail-hero img');
    const count = await images.count();
...
test.describe('InsightsDetailHero — Accessibility', () => {
  test('[IDH-010] @a11y @wcag22 @regression @smoke InsightsDetailHero passes axe-core scan', async ({ page }) => {
    const pom = new InsightsDetailHeroPage(page);
    await pom.navigate(BASE());
    const results = await new AxeBuilder({ page })
      .include('.cmp-insights-detail-hero')
      .withTags(["wcag2a","wcag2aa","wcag22aa"])
      .analyze();
    expect(results.violations).toEqual([]);
  });

  test('[IDH-011] @a11y @wcag22 @regression @smoke InsightsDetailHero interactive elements meet 24px target size', async ({ page }) => {
    const pom = new InsightsDetailHeroPage(page);
    await pom.navigate(BASE());
    const interactive = page.locator('.cmp-insights-detail-hero a, .cmp-insights-detail-hero button, .cmp-insights-detail-hero input');
    const count = await interactive.count();
    for (let i = 0; i < count; i++) {
  ...
  test('[IDH-012] @a11y @wcag22 @regression @smoke InsightsDetailHero focus is not obscured by sticky elements', async ({ page }) => {
    const pom = new InsightsDetailHeroPage(page);
    await pom.navigate(BASE());
    const focusable = page.locator('.cmp-insights-detail-hero a, .cmp-insights-detail-hero button, .cmp-insights-detail-hero input');
...
test.describe('InsightsDetailHero — AEM Dialog Configuration', () => {
  ...
  test('[IDH-013] @author @regression @smoke @smoke InsightsDetailHero dialog has helpPath configured', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/insights-detail-hero/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
    expect(response.ok(), 'InsightsDetailHero GA dialog overlay not found — component may be missing _cq_dialog').toBe(true);
    const dialog = await response.json();
    expect(dialog.helpPath, 'InsightsDetailHero dialog missing helpPath property').toBeTruthy();
  });

  test('[IDH-014] @author @regression @smoke InsightsDetailHero helpPath points to correct component details page', async ({ page }) => {
    const dialogUrl = `${BASE()}/apps/ga/components/content/insights-detail-hero/_cq_dialog.1.json`;
    const response = await page.request.get(dialogUrl);
