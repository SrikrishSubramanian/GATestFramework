...
test.describe('InsightsDetailHero — Visual Regression', () => {
  test('@visual @regression Desktop screenshot matches baseline', async ({ page }) => {
    ...
    const el = page.locator('.cmp-insights-detail-hero').first();
    await expect(el).toHaveScreenshot('insights-detail-hero-desktop.png', {
  ...
  test('@visual @regression @mobile Mobile screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-insights-detail-hero').first();
    await expect(el).toHaveScreenshot('insights-detail-hero-mobile.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
  ...
  test('@visual @regression Tablet screenshot matches baseline', async ({ page }) => {
    ...
    await pom.navigate(BASE());
    const el = page.locator('.cmp-insights-detail-hero').first();
    await expect(el).toHaveScreenshot('insights-detail-hero-tablet.png', {
      maxDiffPixelRatio: 0.001,
      animations: 'disabled',
    });
  });
