...
export class EnhancedRelatedContentPage {
  ...
  async navigate(baseUrl: string) {
    try {
      return await this.page.goto(`${baseUrl}/content/global-atlantic/style-guide/components/enhanced-related-content.html?wcmmode=disabled`, { waitUntil: 'domcontentloaded' });
    } catch (err) {
      ...
      if (err instanceof Error && (err.message.includes('interrupted by another navigation') || err.message.includes('NS_ERROR_ABORT'))) {
        return this.page.goto(`${baseUrl}/content/global-atlantic/style-guide/components/enhanced-related-content.html?wcmmode=disabled`, { waitUntil: 'domcontentloaded' });
      }
      throw err;
    }
  }
